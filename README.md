...
## 4. Hiring Plan — Команда MVP

> Минимальная команда для запуска и проверки гипотезы. Принцип: один человек — одна зона ответственности.

| # | Роль | Зона ответственности 1 | Зона ответственности 2 |
|---|------|------------------------|------------------------|
| 1 | **Product Manager** | Формирует бэклог, приоритизирует задачи по RICE, принимает готовые фичи | Ведёт коммуникацию с B2B-партнёрами, собирает обратную связь |
| 2 | **Backend-разработчик** | Проектирует и реализует API (FastAPI): консультации, подписки, триаж | Интеграции с ЮKassa, Push APNS/FCM, Telegram Bot API |
| 3 | **Mobile-разработчик** | Разрабатывает мобильное приложение (React Native): чат, видео, подписки | Обеспечивает публикацию в App Store и Google Play |
| 4 | **Frontend-разработчик** | Разрабатывает веб-кабинет ветеринара и B2B-партнёра (React) | Разрабатывает admin-панель: верификация врачей, white-label настройки |
| 5 | **UX/UI-дизайнер** | Проектирует пользовательские сценарии, wireframes, дизайн-систему | Готовит white-label шаблоны брендинга для B2B-партнёров |
| 6 | **DevOps / SRE** | Настраивает CI/CD, Docker, nginx, облачную инфраструктуру (Yandex Cloud) | Мониторинг, алерты, резервное копирование БД |
| 7 | **QA-инженер** | Ручное тестирование критических сценариев (чат, оплата, видео) | Автотесты на API, регрессионное тестирование перед релизом |

**Итого: 7 человек** — достаточно для MVP без раздувания команды.

---

## 5. Development Framework — Методология

### Выбор: **Scrum с двухнедельными спринтами**

**Тезис 1 — Неопределённость на старте требует коротких итераций**
VetCare выходит на новый рынок телемедицины для животных, где гипотезы (конверсия в подписку, удержание, поведение ветеринаров) ещё не проверены. Двухнедельный спринт даёт реальную обратную связь от пользователей каждые 14 дней и позволяет разворачивать приоритеты без потери месяцев работы.

**Тезис 2 — B2B-партнёры требуют предсказуемых дат**
Зоосети и ветклиники планируют маркетинговые активности заранее. Sprint Review каждые две недели даёт партнёру видимость прогресса и конкретные даты фич — это доверие и управляемые ожидания без водопада.

**Тезис 3 — Маленькая команда = быстрый Scrum без бюрократии**
7 человек помещаются в один Daily Standup за 15 минут. Не нужен отдельный Scrum Master — роль берёт PM. Kanban не даёт ритма и дедлайнов; SAFe избыточен для стартапа. Scrum — золотая середина между хаосом и корпоративной тяжестью.

---

## 6. Team Rituals — Командные ритуалы

| Встреча | Цель | Частота | Длительность | Участники |
|---------|------|---------|--------------|-----------|
| **Daily Standup** | Синхронизация: что сделал вчера, что делаю сегодня, есть ли блокеры | Ежедневно | 15 мин | Вся команда |
| **Sprint Planning** | Выбор задач из бэклога на спринт, оценка в story points, распределение | 1 раз в 2 недели (начало спринта) | 2 часа | Вся команда |
| **Sprint Review** | Демонстрация готовых фич стейкхолдерам и партнёрам, приёмка задач | 1 раз в 2 недели (конец спринта) | 1 час | Команда + партнёры |
| **Sprint Retrospective** | Что шло хорошо, что мешало, какие процессы улучшить | 1 раз в 2 недели (после Review) | 45 мин | Вся команда |
| **Backlog Refinement** | Уточнение и декомпозиция задач следующего спринта, актуализация приоритетов | 1 раз в неделю | 1 час | PM + Tech Lead |
| **1-on-1** | Обратная связь по росту, мотивации, проблемам каждого участника команды | 1 раз в 2 недели | 30 мин | PM + каждый член команды |
...
### Риск 1 — Отказ B2B-партнёра от интеграции 🔴

| Тип | Вероятность | Влияние | Уровень |
|-----|-------------|---------|---------|
| Внешний | Высокая | Высокое | 🔴 Высокий |

**Описание:** Зоосеть передумывает после переговоров или затягивает запуск — вся white-label разработка оказывается под угрозой.

**Стратегия реагирования:** Подписывать LOI (letter of intent) до старта разработки white-label. Параллельно вести переговоры с 3+ партнёрами одновременно — не делать ставку на одного.

---

### Риск 2 — Нехватка ветеринаров на платформе 🔴

| Тип | Вероятность | Влияние | Уровень |
|-----|-------------|---------|---------|
| Внутренний | Средняя | Высокое | 🔴 Высокий |

**Описание:** Дефицит врачей в онлайне приводит к SLA >5 минут, пользователи уходят и не возвращаются.

**Стратегия реагирования:** Сформировать пул из 20–30 ветеринаров до публичного запуска. Ввести систему приоритетной оплаты за дежурство в ночное время и выходные дни.

---

### Риск 3 — Регуляторный риск 🟡

| Тип | Вероятность | Влияние | Уровень |
|-----|-------------|---------|---------|
| Внешний | Низкая | Высокое | 🟡 Средний |

**Описание:** Изменение законодательства о телемедицине для животных, ужесточение лицензионных требований или запрет дистанционных консультаций.

**Стратегия реагирования:** Нанять юридического консультанта по ветеринарному праву на старте. Проектировать платформу так, чтобы телемедицинский модуль можно было отключить без потери остального продукта.

---

### Риск 4 — Технический сбой во время консультации 🟡

| Тип | Вероятность | Влияние | Уровень |
|-----|-------------|---------|---------|
| Внутренний | Средняя | Среднее | 🟡 Средний |

**Описание:** Обрыв видео или чата в критический момент приводит к потере доверия пользователя и негативным отзывам.

**Стратегия реагирования:** Мониторинг uptime 99.9% (Grafana + алерты). Автоматическое восстановление сессии при reconnect. Компенсация пользователю бесплатной консультацией при подтверждённом сбое по вине платформы.

---

### Риск 5 — Утечка персональных и медицинских данных 🔴

| Тип | Вероятность | Влияние | Уровень |
|-----|-------------|---------|---------|
| Внутренний | Низкая | Критическое | 🔴 Высокий |

**Описание:** Нарушение 152-ФЗ и требований к медицинским данным — штрафы, дискредитация платформы, потеря всех партнёров.

**Стратегия реагирования:** E2EE переписки, токенизация платёжных данных (PCI DSS), хранение данных только в российских дата-центрах. Регулярный penetration testing, политика минимальных прав доступа для каждой роли.

---

### Риск 6 — Выход крупного конкурента 🟡

| Тип | Вероятность | Влияние | Уровень |
|-----|-------------|---------|---------|
| Внешний | Средняя | Среднее | 🟡 Средний |

**Описание:** Яндекс, VK или крупный агрегатор ветклиник запускает аналогичный сервис с большим маркетинговым бюджетом.

**Стратегия реагирования:** Ускорить заключение эксклюзивных контрактов с топ-3 зоосетями. Углублять white-label интеграцию — чем глубже продукт встроен в бренд партнёра, тем выше стоимость переключения на конкурента.
